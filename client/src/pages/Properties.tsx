...
import { Property, PropertyStatus, DashboardData, PropertyData } from '@/lib/types';
import { apiRequest } from '@/lib/queryClient';
import { motion, AnimatePresence } from 'framer-motion';
import { PlusCircle, FileUp, SnowflakeIcon } from 'lucide-react';
...
const Properties = () => {
  ...
  const [riskAssessmentRate, setRiskAssessmentRate] = useState(73.5);
  const [highRiskProperties, setHighRiskProperties] = useState(0);
  ...
  useEffect(() => {
    const getAllProperties = async () => {
      ...
      for (let i = 0; i < result.data.length; i++) {
        const propertyComplianceStatus = result.data[i].compliance.currentStatus;
        const propertyRiskLevel = result.data[i].compliance.riskLevel;
        const propertyRiskReason = result.data[i].compliance.riskReason;

        console.log("Property Compliance Status", propertyComplianceStatus);
        console.log("Property Risk Level", propertyRiskLevel);
        console.log("Property Risk Reason", propertyRiskReason);

        if (propertyComplianceStatus === 'Compliant') {
          console.log("Property is Compliant");
          complianceCount++;
        }

        if (propertyComplianceStatus === 'at-risk') {
          console.log("Property is At Risk");
          atRiskCount++;
        }

        if (propertyComplianceStatus === 'non-compliant') {
          console.log("Property is Non-Compliant");
          nonCompliantCount++;
        }
      }

      setComplianceRate(complianceCount);
      setHighRiskProperties(nonCompliantCount);
      setRiskAssessmentRate(atRiskCount);
      console.log("Compliance Rate", complianceCount);
  ...
  const allProperties2 = Array.isArray(propertyData) ? propertyData : [];
  ...
  const filteredProperties2 = allProperties2.filter((property: PropertyData) => {
    const matchesSearch = searchTerm === '' ||
      property.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      property.address.toLowerCase().includes(searchTerm.toLowerCase()) ||
      (property.description && property.description.toLowerCase().includes(searchTerm.toLowerCase()));
    const matchesStatus = statusFilter === 'all' || property.compliance.currentStatus === statusFilter;
  ...
  const propertyCounts = {
    all: totalProperties, //allProperties.length,
    // Use dashboard complianceRate to calculate compliant properties
    compliant: stats.complianceRate, //Math.round(((stats.complianceRate || 0) / 100) * totalProperties),
    // At Risk: Use dashboard risk assessment to calculate at-risk properties
    atRisk: stats.riskAssessmentRate, // Math.round(((100 - riskAssessmentRate) / 100) * totalProperties),
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <motion.div
          ...
          className={`group overflow-hidden rounded-xl shadow-sm transition-all cursor-pointer ${statusFilter === 'all'
        ...
        <motion.div
          ...
          className={`group overflow-hidden rounded-xl shadow-sm transition-all cursor-pointer ${statusFilter === 'Compliant'
        ...
        <motion.div
          ...
          className={`group overflow-hidden rounded-xl shadow-sm transition-all cursor-pointer ${statusFilter === 'at-risk'
        ...
        <motion.div
          ...
          className={`group overflow-hidden rounded-xl shadow-sm transition-all cursor-pointer ${statusFilter === 'non-compliant'
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm mb-8">
        ...
        <div className="flex justify-between items-center">
          <div className="text-sm text-gray-600">
            <span className="font-medium">{filteredProperties2.length}</span> {filteredProperties2.length === 1 ? 'property' : 'properties'} found
      ...
      {/* {totalProperties > 0 ? (
        <div>property found {totalProperties}</div>
      ) : (
      ...
      {filteredProperties2.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <AnimatePresence mode="wait" initial={false}>
            {filteredProperties2.map((property: PropertyData, index) => (
              <ContentTransition
                key={property._id}
